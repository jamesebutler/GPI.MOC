...
Public Class clsViewSearch
    ...
    Public Property EventCategory() As String
        ...
        End Get
        Set(ByVal value As String)
            mEventCategory = value
        End Set
    End Property
    Public Property EquipmentCategory() As String
        Get
            Return mEquipmentCategory
        End Get
        Set(ByVal value As String)
            mEquipmentCategory = value
        End Set
    End Property
    ...
    Private Property SearchDTString() As String
        Get
            Return mSearchDTString
        End Get
        Set(ByVal value As String)
            mSearchDTString = value
        End Set
    End Property
    ...
    Public Function GetDataTableString() As String
        GetIncidentListing(True)
        Return SearchDTString
    End Function

    Public Function exportData(ByVal myDataSet As DataSet) As String
        ...
        Try
            ...
            Dim DEF_EXPORT_SEP As String = ","                         ' seperator chacarter between values
            Dim DEF_EXPORT_VAL As String = """"                         ' chararcter a value is placed into

            'write out header
            For Each col As DataColumn In myDataSet.Tables(0).Columns
                sheader = sheader & DEF_EXPORT_VAL & col.ColumnName & DEF_EXPORT_VAL & DEF_EXPORT_SEP
            Next
            ...
            For Each row As DataRow In myDataSet.Tables(0).Rows
                ...
                For i As Integer = 0 To arr.Length - 1
                    If arr(i).ToString().IndexOf(",") > 0 Then
                        sline = sline & DEF_EXPORT_VAL & arr(i).ToString() & DEF_EXPORT_VAL & DEF_EXPORT_SEP
                    Else
                        sline = sline & arr(i).ToString() & ","
                    End If
                Next
                sbody = sbody & sline.Substring(0, sline.Length - 1) & vbCrLf
                sline = ""
            Next
            sbody = sheader & sbody
    ...
    Private Sub GetIncidentListing(Optional ByVal createDataTable As Boolean = False)
        ...
        Try
            ...
            '    ActiveFlag = "N"
            'End If
            RI.SharedFunctions.InsertLoggingRecord("GetIncidentListing ", "START")
            ...
            param = New OracleParameter
            param.ParameterName = "in_EVENTCATEGORY"
            param.OracleDbType = OracleDbType.VarChar
            param.Direction = Data.ParameterDirection.Input
            param.Value = mEventCategory
            paramCollection.Add(param)

            param = New OracleParameter
            param.ParameterName = "in_EQUIPMENTCATEGORY"
            param.OracleDbType = OracleDbType.VarChar
            param.Direction = Data.ParameterDirection.Input
            param.Value = mEquipmentCategory
            paramCollection.Add(param)

            param = New OracleParameter
            param.ParameterName = "in_COMPONENTCATEGORY"
            param.OracleDbType = OracleDbType.VarChar
            param.Direction = Data.ParameterDirection.Input
            param.Value = mComponentCategory
            paramCollection.Add(param)
            ...
            If createDataTable = True Then
                ...
                ds = RI.SharedFunctions.GetDSFromPackage(paramCollection, "Reladmin.RIVIEWGPI.IncidentListingDDL", "", 0)

                mSearchDTString = exportData(ds)
                'Me.SearchDS = ds
                'Me.SearchData = CType(ds.Tables(0).CreateDataReader, OracleDataReader)
            End If
            Dim dr As Devart.Data.Oracle.OracleDataReader = RI.SharedFunctions.GetOraclePackageDR(paramCollection, "Reladmin.RIVIEWGPI.IncidentListingDDL")
            ...
            If dr IsNot Nothing Then
                If createDataTable = True Then
                    ...
                    Dim dateLoad1 As Date = DateTime.Now

                    dt.Load(dr)

                    Dim dateLoad2 As Date = DateTime.Now
                    Dim durationload As TimeSpan = dateLoad2 - dateLoad1
                    RI.SharedFunctions.InsertLoggingRecord("GetIncidentListing", "Time to Load Datatable: " & durationload.ToString)
            ...
            RI.SharedFunctions.InsertLoggingRecord("GetIncidentListing", "Duration: " & duration.ToString)
    ...
    Private mEventCategory As String
    Private mEquipmentCategory As String
    Private mComponentCategory As String
    Private mSearchDTString As String
...
Public Class clsViewUpdate
    ...
    Public ReadOnly Property EventCategory() As clsData
        Get
            Return mEventCategory
        End Get
    End Property
    Public ReadOnly Property EquipmentCategory() As clsData
        Get
            Return mEquipmentCategory
        End Get
    End Property
    Public ReadOnly Property ComponentCategory() As clsData
        Get
            Return mComponentCategory
        End Get
    End Property
    ...
    Private mEventCategory As New clsData
    Private mEquipmentCategory As New clsData
    Private mComponentCategory As New clsData
