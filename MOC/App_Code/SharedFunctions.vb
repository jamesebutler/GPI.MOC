...
Namespace RI
    ...
    Public Class SharedFunctions
        ...
        Public logging As Boolean = CBool(ConfigurationManager.AppSettings("Logging"))
        Public erroremail As String = ConfigurationManager.AppSettings("ErrorEmail")


        Public Function IsAuthenticatedGPI(ByVal username As String) As Boolean
        ...
        Shared Function IsDevloperUser(ByVal name As String) As Boolean
            name = name.ToLower
            Dim adminUsers As New ArrayList
            Dim admUsrArr = ConfigurationManager.AppSettings("Developers").ToString.ToLower.Split(New Char() {","c})
            adminUsers.AddRange(admUsrArr)
            Return adminUsers.Contains(name)
        End Function
        ...
        Public Shared Sub ResponseRedirect(ByVal url As String) ', Optional ByVal endResponse As Boolean = False)
            RI.SharedFunctions.InsertRILoggingRecord("SharedFunctions.vb", "ResponseRedirect")
            Try
        ...
        Public Shared Function CallDROraclePackage(ByRef parms As OracleParameterCollection,
                                                   ByVal packageName As String) As String 'OracleDataReader
            Dim conCust As OracleConnection = Nothing
            Dim cmdSQL As OracleCommand = Nothing
            Dim connection As String = String.Empty
            Dim provider As String = String.Empty
        ...
        Public Shared Function GetAuthLevelAdmin(ByVal user As String, ByVal site As String) As String
            Dim paramCollection As New OracleParameterCollection
            Dim param As New OracleParameter
            Dim ds As System.Data.DataSet = Nothing
            Dim ret As String = String.Empty
            ...
            Try
                ...
                ds = RI.SharedFunctions.GetDSFromPackage(paramCollection, "Reladmin.newmocgpi.GetAuthLevelAdmin", key, 8)
        ...
        Shared Sub SendEmail(ByVal toaddress As String,
            ...
            RI.SharedFunctions.InsertRILoggingRecord("SharedFunctions.vb", subject + " SendEmail")
                ...
                If OkToSend = True Then
                    ...
                    Do While emailTryCount < 5 And emailSuccess = False
                        ...
                        Catch ex As SmtpException
                            ...
                            RI.SharedFunctions.InsertAuditRecord("www. Error", "The following email was not sent - Retry(" & emailTryCount & ") -  " & Mid(body, 1, 3000) & "<br> Recipients:" & Mid(inputAddress.ToString, 1, 500))
                        Finally
                            client = Nothing
                        End Try
        ...
        Public Shared Function isEmail(ByVal inputEmail As String) As Boolean
            ...
            End If
        End Function
        Public Shared Sub InsertLoggingRecord(ByVal sourceName As String, ByVal sourceMessage As String)

            '=========================================================================================================================
            'Added: 4/10/2020
            'Author: James Butler
            'Description:  Added to log anything within the program.  Ex. can be use to log how long a function/sub is taking.
            'Check webcong "Logging" to turn on or off.
            '=========================================================================================================================
            If CBool(ConfigurationManager.AppSettings("Logging")) Then
                ...
                Try
                    ...
                    param.OracleDbType = OracleDbType.VarChar
                    param.Direction = Data.ParameterDirection.Input
                    param.Value = sourceMessage
                    paramCollection.Add(param)

                    Dim returnStatus As String = RI.SharedFunctions.CallDROraclePackage(paramCollection, "Reladmin.LOGGING.InsertLoggingRecord")
        ...
        Public Shared Sub InsertRILoggingRecord(ByVal sourceName As String, ByVal sourceMessage As String, Optional ByVal log As String = "")
            ...
            'Author: James Butler
            'Description:  Added to log anything within the program.  Ex. can be use to log how long a function/sub is taking.
            'Check webconfig "Logging" to turn on or off.
            'check webconfig to see if only sourename is logged
            '=========================================================================================================================
            If CBool(ConfigurationManager.AppSettings("Logging")) Then
                ...
                If ConfigurationManager.AppSettings("LogAll") = "" Then
                    'log everything
                    param = New OracleParameter
                    param.ParameterName = "in_routine"
                    param.OracleDbType = OracleDbType.VarChar
                    param.Direction = Data.ParameterDirection.Input
                    param.Value = sourceName
                    paramCollection.Add(param)
                    ...
                    param = New OracleParameter
                    param.ParameterName = "in_message"
                    param.OracleDbType = OracleDbType.VarChar
                    param.Direction = Data.ParameterDirection.Input
                    param.Value = sourceMessage
                    paramCollection.Add(param)

                ElseIf ConfigurationManager.AppSettings("LogAll") = sourceName Then
                    'only log what is selected
                    param = New OracleParameter
                        param.ParameterName = "in_routine"
                        param.OracleDbType = OracleDbType.VarChar
                        param.Direction = Data.ParameterDirection.Input
                    ...
                    Dim returnStatus As String = RI.SharedFunctions.CallDROraclePackage(paramCollection, "Reladmin.LOGGING.InsertRILoggingRecord")
                    If CDbl(returnStatus) <> 0 Then
                        'Throw New Data.DataException("Error Marking Analysis Complete for " & RINumber)
                    End If
                Catch ex As Exception
        ...
        Public Shared Sub InsertRITraceRecord(ByVal sourceName As String,
                                              ByVal sourceMessage As String,
                                              Optional ByVal sourceTime As String = "")
            ...
            If CBool(ConfigurationManager.AppSettings("Logging")) Then
                ...
                Try
                    ...
                    param = New OracleParameter
                    param.ParameterName = "in_time"
                    param.OracleDbType = OracleDbType.VarChar
                    param.Direction = Data.ParameterDirection.Input
                    param.Value = sourceTime
                    paramCollection.Add(param)
        ...
        Public Shared Sub InsertLoginRecord(ByVal username As String, ByVal rountine As String, ByVal Message As String)
            ...
            Try
                ...
                Dim returnStatus As String = RI.SharedFunctions.CallDROraclePackage(paramCollection, "Reladmin.LOGGING.InsertLoggingracingRecord")
                If CDbl(returnStatus) <> 0 Then
                    'Throw New Data.DataException("Error Marking Analysis Complete for " & RINumber)
                End If
            Catch ex As Exception
        ...
        Public Shared Sub HandleError(Optional ByVal MethodName As String = "RI", Optional ByVal additionalErrMsg As String = "", Optional ByVal excep As Exception = Nothing)
            ...
            Dim redirectURL As String = "~/Help/ErrorPage.aspx"
            Try
                ...
                If le IsNot Nothing Then
                    ...
                    Do While le IsNot Nothing
                        ...
                        errMsg = String.Format(errMsg, RI.CurrentUserProfile.GetCurrentUser, errorCount, HttpContext.Current.Request.ServerVariables("HTTP_USER_AGENT"), HttpContext.Current.Request.Url, Date.Now.ToString("F"), le.ToString, additionalErrMsg)
                        ...
                        For i As Integer = 0 To errMsg.Length Step maxLen
                            ...
                            'email error
                            SendEmail(ConfigurationManager.AppSettings("ErrorEmail"), ConfigurationManager.AppSettings("ErrorEmail"), "ERROR", errValue)
        ...
        Public Shared Function WriteExcelXml(ByRef source As Data.Common.DbDataReader, ByVal excludedFields As ArrayList) As String
            ...
            InsertLoggingRecord("WriteExcelXml START ", "")
            ...
            Dim startExcelXML As String =
                ...
                "<Workbook " &
                "xmlns='urn:schemas-microsoft-com:office:spreadsheet' " & vbCrLf &
                "xmlns:o='urn:schemas-microsoft-com:office:office' " & vbCrLf &
                "xmlns:x='urn:schemas-microsoft-com:office:excel' " & vbCrLf &
                "xmlns:ss='urn:schemas-microsoft-com:office:spreadsheet'>" & vbCrLf &
                "<Styles>" & vbCrLf &
                "<Style ss:ID='Default' ss:Name='Normal'>" & vbCrLf &
                "<Alignment ss:Vertical='Bottom'/>" & vbCrLf &
                "<Borders/>" & vbCrLf &
                "<Font/>" & vbCrLf &
                "<Interior/>" & vbCrLf &
                "<NumberFormat/>" & vbCrLf &
                "<Protection/>" & vbCrLf &
                "</Style>" & vbCrLf &
                "<Style ss:ID='BoldColumn'>" & vbCrLf &
                "<Font x:Family='Swiss' ss:Bold='1'/>" & vbCrLf &
                "</Style>" & vbCrLf &
                "<Style ss:ID=""s21"">" & vbCrLf &
                "<NumberFormat ss:Format=""m/d/yyyy;@""/>" & vbCrLf &
                "</Style>" & vbCrLf &
                "<Style ss:ID=""s23"">" & vbCrLf &
                "<NumberFormat ss:Format=""Standard""/>" & vbCrLf &
                "</Style>" & vbCrLf &
                "<Style ss:ID=""s24""><NumberFormat/></Style>" & vbCrLf &
                "<Style ss:ID=""s26"">" & vbCrLf &
                "<NumberFormat ss:Format=""[$-419]d\ mmm\ yy;@""/></Style>" & vbCrLf &
                "<Style ss:ID=""s28"">" & vbCrLf &
                "<NumberFormat ss:Format=""[ENG][$-409]d\-mmm\-yy;@""/></Style>" & vbCrLf &
                "<Style ss:ID=""s66"">" & vbCrLf &
                "<NumberFormat ss:Format=""[$-416]d\-mmm\-yy;@""/></Style>" & vbCrLf &
                "<Style ss:ID=""s64""><NumberFormat ss:Format=""[$-40C]d\-mmm\-yy;@""/></Style>" & vbCrLf &
                "</Styles>"
            ...
            Do While source.Read()
                ...
                If (newPage) Then
                    ...
                    Do While (z < source.FieldCount)
                        'InsertLoggingRecord("FIELD NAME:   WriteExcelXml - ", source.GetName(z))
                ...
                Do While (y < source.FieldCount)
                    ...
                    Select Case (rowType.ToString)
                        ...
                        Case "System.String"
                            ...
                            '        styleKey = "s64"
                            '    End If
                            '    excelDoc.Append("<Cell ss:StyleID=""" & styleKey & """>" &
            ...
            InsertLoggingRecord("WriteExcelXml", " FIELD COUNT: " + CStr(y.ToString) + "  -  RECORD COUNT: " + CStr(rowCount.ToString))

            InsertLoggingRecord("WriteExcelXml END ", "Duration: " & duration.ToString)
