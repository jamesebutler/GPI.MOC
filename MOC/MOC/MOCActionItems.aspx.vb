...
Partial Class RI_MOCActionItems
    Inherits RIBasePage
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ScriptManager.RegisterClientScriptInclude(Me._udpActionItem, _udpActionItem.GetType, "ActionItems", Page.ResolveClientUrl("~/MOC/MOCActionItems.js"))
    ...
    Private Function GetActionsDRFromPackage() As OracleDataReader
        ...
        Try
            ...
            dr = RI.SharedFunctions.GetOraclePackageDR(paramCollection, "MOCActions.GetActions")
        Catch ex As Exception
    ...
    Protected Sub UpdateActionItems()
        ...
        Dim strTaskDesc, strComments As String
        Dim tbEstDate, tbCompDate As TextBox
        Dim tbTaskDesc, tbComments, tbWO As TextBox
        Dim ddlResource, ddlPriority As DropDownList
        ...
        Try
            ...
            If Me._tblNewRow.Visible = "true" Then
                If Me._tbNewTaskDesc.Text <> Nothing And Me._ddlNewPriority.SelectedValue <> "" And Me._dtNewEstDate.DateValue <> Nothing Then
                    ...
                    Dim status As String = UpdateActionsPackage(strResource, strPriority, strTaskDesc, strComments, 0, strWO, dtEstDate, dtCompDate)
            ...
            For i = 0 To _gvCorrectiveActions.DirtyRows.Count - 1
                ...
                UpdateActionsPackage(strResource, strPriority, strTaskDesc, strComments, intActionNumber, strWO, dtEstDate, dtCompDate)
        ...
        End Try
    End Sub
    Private Function UpdateActionsPackage(ByVal strResource As String, ByVal strPriority As String, ByVal strTaskDesc As String, ByVal strComments As String, ByVal ActionId As String, ByVal strWO As String, ByVal dtEstDate As String, ByVal dtCompDate As String) As String
        ...
        Try
            ...
            status = RI.SharedFunctions.CallDROraclePackage(paramCollection, "MOCactions.ActionItems")
            If status <> "0" Then
        ...
        Try
            ...
            Else
                If Request.ServerVariables("HTTP_HOST").ToLower.Contains("ridev") Then
                    urlHost = "Http://ridev/ri/MOC/"
                ElseIf Request.ServerVariables("HTTP_HOST").ToLower.Contains("ritest") Then
                    urlHost = "Http://ritest/ri/MOC/"
                Else
                    ' urlHost = "Http://ri/ri/MOC/"
                    urlHost = "http://gpiri.graphicpkg.com/MOC/"
                End If
            ...
            Body = Body & "</ul>"
            Body = Body & "<p><br></br><TD><A HREF=" & urlHost & "EnterMOC.aspx?MOCNumber=" & MOCNumber & ">" & Master.RIRESOURCES.GetResourceValue("Click here to Review MOC and Update Action Items") & "</A></TD>" & "</br></TR></p>"
            Body = Body & "</body>"
            Body = Body & "</html>"
            ...
            RI.SharedFunctions.SendEmail(ToAddress, FromAddress, Master.RIRESOURCES.GetResourceValue("New Action Item Assigned to you from MOC ") & " " & MOCNumber, Body, userProfile.FullName)
    ...
    Function GetEmail(ByVal strResource As String) As OracleDataReader
        ...
        Try
            ...
            With cmdSQL
                ...
                param = New OracleParameter
                param.ParameterName = "outMyGlobalEmail"
                param.OracleDbType = OracleDbType.Cursor
                param.Direction = Data.ParameterDirection.Output
                .Parameters.Add(param)
            End With
    ...
    Protected Sub Page_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRender
        ...
        ScriptManager.RegisterStartupScript(Me._udpActionItem, _udpActionItem.GetType, "ResourceList", String.Format(sb.ToString, siteID, Me._ddlNewResource.ClientID), True)
    End Sub
