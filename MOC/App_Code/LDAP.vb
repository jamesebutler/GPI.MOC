...
Namespace RI
    ...
    Public Class CurrentUserProfile
        Private MyGlobalUsername As String = String.Empty
        Private MyGlobalDefaultFacility As String = String.Empty
        Private _Domain As String = "GRAPHICPKG"
        Private _Division As String = String.Empty
        Private MyGlobalInActiveFlag As String = String.Empty
        Private MyGlobalBusType As String = String.Empty
        Private MyGlobalFullName As String = String.Empty
        Private _Groups() As String
        Private _PropertyKeys() As String
        Private _PropertyValues() As String
        Private MyGlobalDistinguishedName As String = String.Empty
        Private _MemberOf As String = String.Empty
        Private MyGlobalEmail As String = String.Empty
        Private MyGlobalProfileTable As String = String.Empty
        Private MyGlobalAuthLevel As String = "NO"
        Private MyGlobalAuthLevelID As Integer = 0
        Private MyGlobalDivestedLocation As Boolean = False
        Private MyGlobalDefaultLanguage As String = ""
        'Private _PageTheme As String = "RIWhite"
        Private _PageTheme As String = "RIBlue"
        ...
        Public ReadOnly Property Email() As String
            Get
                Return MyGlobalEmail
            End Get
        End Property

        Public ReadOnly Property DivestedLocation() As Boolean
            Get
                Return MyGlobalDivestedLocation
            End Get
        End Property

        Public ReadOnly Property DefaultLanguage() As String
            Get
                Return MyGlobalDefaultLanguage
            End Get
        End Property

        Public ReadOnly Property PageTheme() As String
            Get
                Return _PageTheme
            End Get
        End Property
        ...
        Public ReadOnly Property DefaultFacility() As String
            Get
                Return MyGlobalDefaultFacility
            End Get
        End Property
        ...
        Public ReadOnly Property FullName() As String
            Get
                Return MyGlobalFullName
            End Get
        End Property

        Public ReadOnly Property ProfileTable() As String
            Get
                Return MyGlobalProfileTable
            End Get
        ...
        Private Sub BuildProfileTable()
            ...
            Dim tblRow As String = "<tr><th class='ProfileTableDataCellTH' nowrap=nowrap width=300 align=right>{0}</th><td class='ProfileTableDataCell' align=left>{1}</td></tr>"
            With Me
                sb.Append("<table width=99% border=1 class='ProfileTable' cellpadding=0 cellspacing=0>")
                sb.Append(String.Format(tblRow, "Full Name:", .MyGlobalFullName))
                sb.Append(String.Format(tblRow, "User Name:", ._Domain & "\" & .MyGlobalUsername))
                sb.Append(String.Format(tblRow, "Email Address:", .Email))
                sb.Append(String.Format(tblRow, "Site:", ._Siteid))
                sb.Append(String.Format(tblRow, "Default Facility:", .MyGlobalDefaultFacility))
                sb.Append(String.Format(tblRow, "Default Division:", ._Division))
                sb.Append(String.Format(tblRow, "Business Type:", .MyGlobalBusType))
                sb.Append(String.Format(tblRow, "Inactive Flag:", .MyGlobalInActiveFlag))
                sb.Append(String.Format(tblRow, "Group Name:", ._MemberOf))
                sb.Append(String.Format(tblRow, "Divested Location:", .MyGlobalDivestedLocation))
                sb.Append(String.Format(tblRow, "Default Language:", .MyGlobalDefaultLanguage))
                sb.Append(String.Format(tblRow, "Theme:", ._PageTheme))
                sb.Append(String.Format(tblRow, "Admin:", .MyGlobalAuthLevel))

                sb.Append("</table>")
                .MyGlobalProfileTable = sb.ToString
            End With
        End Sub
        ...
        End Sub
             ...
             Public Sub GetUserProfileGPI(ByVal username As String, Optional ByVal password As String = "")
            ...
            Try
                ...
                srch.Filter = "(&(objectClass=user)(samAccountName=" & MyGlobalUsername & "))"

                Dim results As SearchResult = Nothing '= srch.FindOne
                results = TryCast(HttpContext.Current.Session.Item("FindOne_" & MyGlobalUsername), SearchResult)
                ...
                If results IsNot Nothing Then
                    HttpContext.Current.Session.Add("FindOne_" & MyGlobalUsername, results)
                    Dim obj As Object
                    ...
                    If results.Properties.Item("samAccountName").Count > 0 Then
                        MyGlobalUsername = CStr(results.Properties("samAccountName")(0))
                    End If
                    If results.Properties.Item("displayname").Count > 0 Then
                        MyGlobalFullName = CStr(results.Properties("displayname")(0))
                    ElseIf results.Properties.Item("name").Count > 0 Then
                        MyGlobalFullName = CStr(results.Properties("name")(0))
                    End If
                    MyGlobalFullName = MyGlobalFullName.Replace("_0", "")
                    ...
                    If results.Properties.Item("Distinguishedname").Count > 0 Then
                        MyGlobalDistinguishedName = CStr(results.Properties("Distinguishedname")(0))
                        If results.Properties.Item("CN").Count > 0 Then
                            Dim cn As String = CStr(results.Properties.Item("CN")(0))
                            MyGlobalDistinguishedName = MyGlobalDistinguishedName.Replace(cn, cn.Replace("-", "_"))
                        End If
                    End If
                    If results.Properties("mail").Count > 0 Then
                        MyGlobalEmail = CStr(results.Properties("mail")(0))

                    End If
                End If


                System.Web.HttpContext.Current.Session("samaccountname") = (CType(results.Properties("samaccountname")(0), String))
                System.Web.HttpContext.Current.Session("cn") = (CType(results.Properties("cn")(0), String))
                System.Web.HttpContext.Current.Session("sn") = (CType(results.Properties("sn")(0), String))
                System.Web.HttpContext.Current.Session("displayname") = (CType(results.Properties("displayname")(0), String))
                System.Web.HttpContext.Current.Session("givenname") = (CType(results.Properties("givenname")(0), String))
                System.Web.HttpContext.Current.Session("title") = (CType(results.Properties("title")(0), String))
                System.Web.HttpContext.Current.Session("userprincipalname") = (CType(results.Properties("userprincipalname")(0), String))
                System.Web.HttpContext.Current.Session("mail") = (CType(results.Properties("mail")(0), String))
                System.Web.HttpContext.Current.Session("mailnickname") = (CType(results.Properties("mailnickname")(0), String))
                System.Web.HttpContext.Current.Session("physicaldeliveryofficename") = (CType(results.Properties("physicaldeliveryofficename")(0), String))
                System.Web.HttpContext.Current.Session("description") = (CType(results.Properties("description")(0), String))
                System.Web.HttpContext.Current.Session("userAccountControl") = (CType(results.Properties("userAccountControl")(0), String))

                ' sname = Split(System.Web.HttpContext.Current.Session("displayname"), ",")
                ' System.Web.HttpContext.Current.Session("displayname") = sname(1) & " " & sname(0)
        ...
        Private Sub GetUserProfile(ByVal username As String, Optional ByVal password As String = "")
            ...
            Try
                ...
                If results IsNot Nothing Then
                    ...
                    End If
                    If results.Properties("mail").Count > 0 Then
                        MyGlobalEmail = RI.SharedFunctions.ToTitleCase(CStr(results.Properties("mail")(0)), True)
                    End If
                End If
                'Dim ouFacility As String = ""
                'Dim pos As Integer = InStr(MyGlobalDistinguishedName, "-")
                'If pos > 0 Then
                '    ouFacility = Mid(MyGlobalDistinguishedName, pos + 1, 3)
                'End If
                SetProfileDefaults()
                'BuildProfileTable()
            Catch ex As Exception
        ...
        '        ldap = TryCast(HttpContext.Current.Session.Item("LDAPProfile_" & MyGlobalUsername), DevLDAP.LdapProfile)
        '        If ldap Is Nothing Then
        '            ldap = ld.GetUser(MyGlobalUsername, _Domain)
        '            If ldap IsNot Nothing Then
        '                HttpContext.Current.Session.Add("LDAPProfile_" & MyGlobalUsername, ldap)
        '            End If
        '        End If
        '        If ldap IsNot Nothing Then
        '            MyGlobalDistinguishedName = ldap.distinguishedname
        '            MyGlobalUsername = ldap.samaccountname
        '            _Groups = Split(ldap.memberof, ",")
        '            _MemberOf = ldap.memberof
        '            If ldap.displayname.Length > 0 Then
        '                MyGlobalFullName = ldap.displayname
        '            Else
        '                MyGlobalFullName = ldap.name
        '            End If
        ...
        '                    MyGlobalEmail = ldap.mail
        '                Else
        '                    'MyGlobalEmail = MyGlobalUsername.Trim & "@ipaper.com"
        '                End If
        '            Else
        '                ' MyGlobalEmail = MyGlobalUsername.Trim & "@ipaper.com"
        '            End If
        ...
        Private Function GetUserDefaultsFromPackage(ByVal user As String, ByVal groupName As String, ByVal ouFacility As String) As OracleDataReader
            ...
            Try
                ...
                If user IsNot Nothing And user.Length > 0 Then
                    ...
                    If userDomain.Length = 2 Then
                        MyGlobalUsername = userDomain(1).ToUpper
                ...
                With cmdSQL
                    ...
                    .CommandText = "ri.UserDefaultsGPI"
                    .CommandType = CommandType.StoredProcedure
                    Dim param As New OracleParameter

                    param.ParameterName = "in_username"
                    param.OracleDbType = OracleDbType.VarChar
                    param.Value = MyGlobalUsername
            ...
            Try
                ...
                If Not dr Is Nothing Then
                    ...
                    If dr.HasRows = True Then
                        ...
                        End If
                        Me.MyGlobalDivestedLocation = True
                    End If
                    dr.NextResult()
                    If dr.HasRows = True Then
                        dr.Read()
                        If Not dr.Item("SiteID") Is Nothing Then
                            ...
                            If MyGlobalInActiveFlag.Length = 0 Then MyGlobalInActiveFlag = CStr(RI.SharedFunctions.DataClean(dr.Item("INACTIVE_FLAG")))
                            MyGlobalBusType = CStr(RI.SharedFunctions.DataClean(dr.Item("BusType")))
                            If RI.SharedFunctions.DataClean(dr.Item("default_language")) IsNot Nothing And RI.SharedFunctions.DataClean(dr.Item("default_language")) IsNot Nothing Then
                                MyGlobalDefaultLanguage = CStr(RI.SharedFunctions.DataClean(dr.Item("default_language")))
                            End If

                            If Not dr.Item("UITHEME") Is Nothing Then

                                _PageTheme = CStr(RI.SharedFunctions.DataClean(dr.Item("UITHEME")))
                            Else
                                _PageTheme = "RIBlue"
                            End If

                            If MyGlobalEmail.Length = 0 Then 'This user doesn't have an email address listed in LDAP
                                If dr.Item("Email") IsNot DBNull.Value Then
                                    MyGlobalEmail = CStr(RI.SharedFunctions.DataClean(dr.Item("Email")))
                                Else
                                    'This code should not be executed
                                    MyGlobalEmail = MyGlobalUsername & "@Graphicpkg.com"
                                    RI.SharedFunctions.InsertAuditRecord("SetProfileDefaults", MyGlobalUsername & " has a null email address in the refEmployee table")
                                End If
                            End If
                        End If
                    Else
                        ...
                        If dr.HasRows = True Then
                            ...
                            If Not dr.Item("SiteID") Is Nothing Then
                                ...
                                If MyGlobalInActiveFlag.Length = 0 Then MyGlobalInActiveFlag = CStr(RI.SharedFunctions.DataClean(dr.Item("INACTIVE_FLAG")))
                                MyGlobalBusType = CStr(RI.SharedFunctions.DataClean(dr.Item("BusType")))
                                If MyGlobalDefaultLanguage.Length = 0 Then
                                    MyGlobalDefaultLanguage = RI.SharedFunctions.DataClean(CStr(dr.Item("default_language")), "en-US")
                                End If
                            End If
                        Else
                            siteID = "AL"
                            If _Division.Length = 0 Then _Division = "ALL"
                            If MyGlobalInActiveFlag.Length = 0 Then MyGlobalInActiveFlag = "N"
                            MyGlobalBusType = "PM"
                        End If
                    ...
                    '        Me.MyGlobalAuthLevel = CStr(dr.Item("authlevel"))
                    '        Me.MyGlobalAuthLevelID = CInt(dr.Item("authlevelid"))
                    '    Else
                    '        Me.MyGlobalAuthLevel = "NO"
                    '        Me.MyGlobalAuthLevelID = 0
                    '    End If
                    'End If
                End If
                If MyGlobalEmail.Length = 0 Then
                    'This code should not be executed b/c we should always have a valid email address
                    MyGlobalEmail = MyGlobalUsername & "@graphicpkg.com"
                    RI.SharedFunctions.InsertAuditRecord("SetProfileDefaults", MyGlobalUsername & " is not listed in the refEmployee table")
                End If

                MyGlobalDefaultFacility = siteID

                Dim AuthLevel As String = RI.SharedFunctions.GetAuthLevelAdmin(MyGlobalUsername, MyGlobalDefaultFacility)
                If AuthLevel <> "" Then
                    MyGlobalAuthLevel = AuthLevel
                End If
        ...
        '                    _Division = CStr(RI.SharedFunctions.DataClean(dr.Item("Division")))
        '                End If
        '                MyGlobalDivestedLocation = True
        '            End If
        '            dr.NextResult()
        '            If dr.HasRows = True Then
        '                dr.Read()
        '                If Not dr.Item("SiteID") Is Nothing Then
        '                    _Siteid = RI.SharedFunctions.DataClean(dr.Item("SiteID")).ToString
        '                    MyGlobalDefaultFacility = _Siteid
        '                    If _Division.Length = 0 Then _Division = CStr(RI.SharedFunctions.DataClean(dr.Item("Division")))
        '                    If MyGlobalInActiveFlag.Length = 0 Then MyGlobalInActiveFlag = CStr(RI.SharedFunctions.DataClean(dr.Item("INACTIVE_FLAG")))
        '                    MyGlobalBusType = CStr(RI.SharedFunctions.DataClean(dr.Item("BusType")))
        '                    If RI.SharedFunctions.DataClean(dr.Item("default_language")) IsNot Nothing And RI.SharedFunctions.DataClean(dr.Item("default_language")) IsNot Nothing Then
